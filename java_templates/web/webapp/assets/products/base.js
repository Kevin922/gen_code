...
(function(){
	...
	XUI.window.alert = function(){
		var build = function(message, title, callback){
			bootbox.dialog({
				message : message,
				title   : title,
				buttons : {
		...
		if(arguments.length == 2){
			build(arguments[0], "弹出窗口", arguments[1]);
		}
		if(arguments.length == 3){
			build(arguments[0], arguments[1] , arguments[2]);
		}
		if(arguments.length == 1){
			build(arguments[0], "弹出窗口", null);
		}
	...
	XUI.window.confirm = function(){
		...
		if(arguments.length == 2){
			build(arguments[0], "确认窗口", arguments[1]);
		}
		if(arguments.length == 3){
			build(arguments[0], arguments[1] , arguments[2]);
		}
	...
	XUI.window.openDialog = function(content,conf){
		...
		//$(temp).children(0).css("width", "-moz-min-content");
		//$(temp).draggable({handle: $(temp).children(0),containment:'window'});
	};
	//关闭顶层窗口
...
(function(){
	...
	XUI.ajax["delete"] = function(url,params,config) {
		...
		if(window.confirm(cfg.confirm)) {
			XUI.ajax.send(url,params,"DELETE","",config);
		}
	};
...
(function(){
	XUI.Page = function(page){
		var location = window.location;
		var form = $("form:first");
		/*if(form.length < 1){
		}*/
		form.attr("action", location);
		//form.append("<input type='hidden' name='pageId' value='"+page+"'>");
		$("input[name='currentPage']").val(page);
		form.submit();
	};

    XUI.ChangePage = function(){
        var input = $("#changePage");
        var page=input.val();
        if(/^\d+$/.test(page)){
            var max = input.attr("max");
            var pageNum = new Number(page);
            var maxNum = new Number(max);
            if(pageNum<=0){
                alert("请输入正整数");
                return;
            }
            ...
            var curr = input.attr("curr");
            var currNum = new Number(curr);
            if(currNum.toString()==pageNum.toString()){
                alert("当前页就是你需要转到的页码。");
                return;
            }
...
(function(){
	XUI.form.submit = function(action, method, data){
		var form = $("<form></form>");
		form.attr("action", action);
		form.attr("method", method);
		$.each(data, function(i, n){
			form.append("<input type='hidden' name='"+n.name+"' value='"+ n.value +"'>");
		});
		form.css("display", "none");
		form.appendTo("body");
		form.submit();
	};
    XUI.form.requery = function(){
        var location = window.location;
        var form = $("form:first");
        form.attr("action", location);
        form.submit();
